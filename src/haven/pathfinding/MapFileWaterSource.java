...
public final class MapFileWaterSource implements WaterwayRoutePlanner.Source {
    ...
    public WaterwayRoutePlanner.Cell cell(Coord tile) {
        if(tile == null) return WaterwayRoutePlanner.Cell.UNKNOWN;
        String name = terrain(tile);
        if(TerrainPolicy.isUnknownTile(name)) return WaterwayRoutePlanner.Cell.UNKNOWN;
        if(!TerrainPolicy.isWaterTile(name)) return WaterwayRoutePlanner.Cell.LAND;
        return TerrainPolicy.isShallowWaterTile(name)
            ? WaterwayRoutePlanner.Cell.SHALLOW : WaterwayRoutePlanner.Cell.DEEP;
    }
