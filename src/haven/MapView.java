...
public class MapView extends PView implements DTarget, Console.Directory, Widget.CursorQuery.Handler {
	...
	final PlobSnap snap = new PlobSnap();

	public void adjust(Plob plob, Coord pc, Coord2d mc, int modflags) {
	    boolean nomod = (modflags & UI.MOD_SHIFT) == 0;
	    boolean snapping = nomod && CFG.PLOB_SNAP.get();
	    Gob pl = plob.mv().player();
	    boolean autorot = (pl != null) && !freerot;
	    double sa = autorot ? (Math.round(plob.rc.angle(pl.rc) / (Math.PI / 2)) * (Math.PI / 2)) : plob.a;
	    /* Hold the angle for as long as the ghost has an edge: a footprint
	     * that keeps turning to face the player cannot stay flush against
	     * anything, and flips on its own as you slide it past yourself. */
	    double na = (snapping && caught) ? plob.a : sa;
	    Coord2d nc = null;
	    if(snapping) {
		nc = snap.snap(plob, mc, na);
		caught = (nc != null);
		if(!caught)
		    na = sa;
	    } else {
	    ...
	    if(nc == null) {
		/* Unsnapped placement is the smooth one, the same as holding
		 * shift -- the tile grid is only for when snapping is turned
		 * off altogether. */
		if(nomod && !CFG.PLOB_SNAP.get())
	...
	Console.setscmd("placesnap", (cons, args) -> {
	    if(args.length < 2) {
		cons.out.printf("grab %.2f, dead zone %.2f (a tile is %.0f)%n", PlobSnap.capture, PlobSnap.deadzone, MCache.tilesz.x);
		if(CFG.DEBUG_PLOB_SNAP.get())
		    cons.out.println("last placement: " + PlobSnap.diag);
		else
		    cons.out.println("last placement: not recorded (turn on placement-snap diagnostics in Options > Debug)");
		return;
	    }
	    if((PlobSnap.capture = Double.parseDouble(args[1])) < 0)
		PlobSnap.capture = 0;
	    Utils.setprefd("plobsnapdist", PlobSnap.capture);
	    if(args.length > 2) {
		if((PlobSnap.deadzone = Double.parseDouble(args[2])) < PlobSnap.capture)
		    PlobSnap.deadzone = PlobSnap.capture;
		Utils.setprefd("plobsnapdead", PlobSnap.deadzone);
	    }
	});
