 ...
 */
public class FishZonePicker implements MapView.Grabber {
    private static volatile FishZonePicker active;
    ...
    static {
        ROLE_RGB.put(FishSpitRoastZoneStore.ROLE_INPUT,  new Integer[]{80, 170, 255});
        ROLE_RGB.put(FishSpitRoastZoneStore.ROLE_FIRE,  new Integer[]{255, 120, 40});
        ROLE_RGB.put(FishSpitRoastZoneStore.ROLE_OUTPUT, new Integer[]{100, 220, 120});
    }
    ...
    private FishZonePicker(MapView map, Integer[] previewRgb, Consumer<Area> onPicked) {
        this.map = map;
    ...
    public static synchronized void start(MapView map, Integer[] previewRgb, Consumer<Area> onPicked) {
        cancel();
        FishZonePicker picker = new FishZonePicker(map, previewRgb, onPicked);
        active = picker;
        map.grab(picker.xl);
    }
    ...
    public boolean mmouseup(Coord mc, int button) {
        ...
        synchronized(FishZonePicker.class) {
            if(active == this) {
                map.release(xl);
                active = null;
            }
