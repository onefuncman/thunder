#!/bin/sh
# Navigation Lab (pathfinding tree). Separate jar and DevControl port from regular Thunder.

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
JAVA=/usr/lib/jvm/java-21-openjdk/bin/java
CRASH_DIR="$ROOT/bin/dev-snapshots/crashes"
JAR="$ROOT/bin/navigation-lab.jar"
...
exec "$JAVA" \
    ...
    -Dhaven.dev.control.port="${HAVEN_DEV_CONTROL_PORT:-18762}" \
    -Dhaven.autologin="${HAVEN_AUTOLOGIN:-false}" \
    -Dhaven.autoplay="${HAVEN_AUTOPLAY:-}" \
    -jar navigation-lab.jar "$@"
